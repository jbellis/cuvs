...
-> vpq_dataset_coarse<MathT, IdxT>
{
    ...
    // Allocate memory for VQ codes
    auto vq_codes = raft::make_device_matrix<uint32_t, IdxT>(res, n_rows, 1);

    // Get CUDA stream
    auto stream = raft::resource::get_cuda_stream(res);

    // Process dataset in batches if necessary
    constexpr IdxT kReasonableMaxBatchSize = 65536;
    IdxT max_batch_size = std::min<IdxT>(n_rows, kReasonableMaxBatchSize);
    ...
    for (const auto& batch : cuvs::spatial::knn::detail::utils::batch_load_iterator(
            ...
            stream,
            rmm::mr::get_current_device_resource())) {
        auto batch_view = raft::make_device_matrix_view<const float, IdxT, raft::row_major>(
                batch.data(), batch.size(), dim);
        auto labels = predict_vq<uint32_t>(res, batch_view, raft::make_const_mdspan(vq_code_book.view()));

        // Copy labels to vq_codes
        raft::copy(vq_codes.data_handle() + batch.offset(),
                   labels.data_handle(),
                   batch.size(),
                   stream);
    }
    ...
    return vpq_dataset_coarse<MathT, IdxT>{std::move(vq_code_book), std::move(vq_codes)};
}
}  // namespace cuvs::neighbors::detail
